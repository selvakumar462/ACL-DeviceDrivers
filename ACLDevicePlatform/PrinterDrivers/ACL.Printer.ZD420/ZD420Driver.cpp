...
namespace ZD420 {
    ...
    bool ZD420Driver::Connect(const std::string& connectionString) {
        ...
        if (!SendCommand("^XA^HON^XZ", response, 2000)) {
            m_lastError = "Failed to communicate with ZD420 printer";
            m_status = ACLDevicePlatform::Core::DeviceStatus::Error;
            return false;
        }
    ...
    bool ZD420Driver::Send(const std::string& data) {
        ...
        if (!m_activeComm->Send(data)) {
            m_lastError = "Failed to send data to ZD420 printer: " + m_activeComm->GetLastError();
            m_status = ACLDevicePlatform::Core::DeviceStatus::Error;
            return false;
        }
    ...
    std::string ZD420Driver::GetModel() const {
        return "ZD420";
    }

    ACLDevicePlatform::Core::DeviceType ZD420Driver::GetDeviceType() const {
        return ACLDevicePlatform::Core::DeviceType::Printer;
    }

    ACLDevicePlatform::Core::DeviceStatus ZD420Driver::GetStatus() const {
        return m_status;
    }

    std::string ZD420Driver::GetLastError() const {
        return m_lastError;
    }

    bool ZD420Driver::Initialize() {
        ...
        if (!SendCommand(initCommands, response, 1000)) {
            m_lastError = "Failed to initialize ZD420 printer";
            return false;
        }
    ...
    bool ZD420Driver::PrintLabel(const std::string& labelContent) {
    ...
    std::string ZD420Driver::GetPrinterStatus() {
        if (m_status != ACLDevicePlatform::Core::DeviceStatus::Connected) {
    ...
    bool ZD420Driver::PerformSelfTest() {
        if (m_status != ACLDevicePlatform::Core::DeviceStatus::Connected) {
            m_lastError = "Printer not connected";
            return false;
        }
    ...
    bool ZD420Driver::InitializeCommunication(const std::string& connectionString) {
    ...
    bool ZD420Driver::SendCommand(const std::string& command, std::string& response, int timeoutMs) {
        ...
        // For ZD420, most ZPL commands don't return responses, but status commands do
    ...
    bool ZD420Driver::ValidateResponse(const std::string& response) {
        // Basic validation - check if response contains expected content
        // For ZD420, responses are typically simple status indicators
